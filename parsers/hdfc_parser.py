...
def parse_hdfc(text):
    data = {}
    data["card_variant"] = re.search(r'(REGALIA|MILLENNIA|DINERS CLUB)', text, re.I)
    data["card_variant"] = data["card_variant"].group(1).upper() if data["card_variant"] else None

    data["card_last4"] = re.search(r'Card No\.\s*XXXX-XXXX-XXXX-(\d{4})', text, re.I)
    ...
    data["new_balance"] = re.search(r'Total Amount Due[:\s]*₹?\s*([\d,]+\.\d{2})', text, re.I)
