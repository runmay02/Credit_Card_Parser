...
def parse_chase(text):
    data = {}
    data["card_variant"] = re.search(r'(REWARDS|PLATINUM|SIGNATURE)', text, re.I)
    data["card_variant"] = data["card_variant"].group(1).upper() if data["card_variant"] else None

    data["card_last4"] = re.search(r'Account Number[:\s]*X{2,}\s*(\d{4})', text, re.I)
    data["card_last4"] = data["card_last4"].group(1) if data["card_last4"] else None

    data["billing_cycle"] = re.search(r'Opening/Closing Date\s*([\d/]+\s*-\s*[\d/]+)', text, re.I)
