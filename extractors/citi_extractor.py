...
def extract_citi_details(raw_content):
    """
    Parses the raw text from a Citi statement to find key details.
    """
    extracted_info = {}

    # Find the card type (e.g., REWARDS, CASHBACK)
    variant_match = re.search(r'(REWARDS|CASHBACK|SIGNATURE)', raw_content, re.I)
    extracted_info["card_type"] = variant_match.group(1).upper() if variant_match else None

    # Find the last 4 digits of the card
    last4_match = re.search(r'Account Number\s*Ending\s*in\s*(\d{4})', raw_content, re.I)
    extracted_info["last_four_digits"] = last4_match.group(1) if last4_match else None

    # Find the billing cycle dates
    period_match = re.search(r'Billing Period[:\s]*([\d/]+\s*-\s*[\d/]+)', raw_content, re.I)
    ...
    balance_match = re.search(r'Total New Balance[:\s]*\$?([\d,]+\.\d{2})', raw_content, re.I)
