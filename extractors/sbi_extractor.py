...
def extract_sbi_details(raw_content):
    """
    Parses the raw text from an SBI statement to find key details.
    """
    extracted_info = {}

    # Find the card type (e.g., SIMPLYCLICK, PRIME)
    variant_match = re.search(r'(SIMPLYCLICK|PRIME|SIGNATURE)', raw_content, re.I)
    extracted_info["card_type"] = variant_match.group(1).upper() if variant_match else None

    # Find the last 4 digits of the card
    last4_match = re.search(r'Card ending\s*in\s*(\d{4})', raw_content, re.I)
