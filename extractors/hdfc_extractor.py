...
def extract_hdfc_details(raw_content):
    """
    Parses the raw text from an HDFC statement to find key details.
    """
    extracted_info = {}

    # Find the card type (e.g., REGALIA, MILLENNIA)
    variant_match = re.search(r'(REGALIA|MILLENNIA|DINERS CLUB)', raw_content, re.I)
    extracted_info["card_type"] = variant_match.group(1).upper() if variant_match else None

    # Find the last 4 digits of the card
    last4_match = re.search(r'Card No\.\s*XXXX-XXXX-XXXX-(\d{4})', raw_content, re.I)
    ...
    balance_match = re.search(r'Total Amount Due[:\s]*₹?\s*([\d,]+\.\d{2})', raw_content, re.I)
