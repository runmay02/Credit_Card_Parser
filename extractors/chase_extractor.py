...
def extract_chase_details(raw_content):
    """
    Parses the raw text from a Chase statement to find key details.
    """
    extracted_info = {}

    # Find the card type (e.g., REWARDS, PLATINUM)
    variant_match = re.search(r'(REWARDS|PLATINUM|SIGNATURE)', raw_content, re.I)
    extracted_info["card_type"] = variant_match.group(1).upper() if variant_match else None

    # Find the last 4 digits of the card
    last4_match = re.search(r'Account Number[:\s]*X{2,}\s*(\d{4})', raw_content, re.I)
    extracted_info["last_four_digits"] = last4_match.group(1) if last4_match else None

    # Find the billing cycle dates
    period_match = re.search(r'Opening/Closing Date\s*([\d/]+\s*-\s*[\d/]+)', raw_content, re.I)
